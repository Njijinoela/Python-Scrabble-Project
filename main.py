...
def human_turn(player, board, wordlist, first_move, calculate_score):
    ...
    while True:
        ...
        try:
            ...
            if scrabble_board.is_valid_move( word, start_row, start_col, direction):
                scrabble_board.place_word( word, start_row, start_col, direction)
                first_move = False
                print("Updated Board:")
            ...
            for col in range(15):
                for direction in ['H', 'V']:
                    if first_move and not (row == 7 and col == 7):
                        continue
                    if scrabble_board.is_valid_move( word, row, col, direction):
                        scrabble_board.place_word(word, row, col, direction)
...
def main():
    ...
    board = scrabble_board.print_board()
    # scrabble_board._apply_special_tile()
